...
HEALTHCHECK --interval=30s --timeout=10s --start-period=5s --retries=3 \
    CMD sh -c 'APP_PORT="${PORT:-${WEBSITES_PORT:-8000}}"; curl -f "http://localhost:${APP_PORT}/api/health/ready" || exit 1'

# Run the backend server
CMD ["sh", "-c", "APP_PORT=\"${PORT:-${WEBSITES_PORT:-8000}}\"; exec uvicorn backend.server:app --host 0.0.0.0 --port \"${APP_PORT}\""]
